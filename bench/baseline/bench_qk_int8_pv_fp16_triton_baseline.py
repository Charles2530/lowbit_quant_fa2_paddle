...
from src.triton.attn_qk_int8_per_block import forward_baseline as forward
from src.triton.attn_qk_int8_per_block_causal import \
    forward_baseline as forward_causal
from utils.benchmark import benchmark_triton_attention_int8_baseline
...
>>>>>>    torch.backends.cuda.enable_mem_efficient_sdp(args.method == "xformers")
    ...
    benchmark_triton_attention_int8_baseline(
...
def main():
    ...
    logger = Logger(
        name="triton_flash_attn_qk_int8_pv_fp16_baseline_" + get_save_name(args),
        log_dir="logs",
        logging_mode=True,
    )
