...
sys.path.append(
    os.path.dirname(os.path.dirname(p=os.path.dirname(p=os.path.abspath(__file__))))
)
from src.triton.quantization.attn_qk_int2_per_block import \
    forward_merging as forward
from src.triton.quantization.attn_qk_int2_per_block_causal import \
    forward as forward_causal
from utils.benchmark import benchmark_triton_attention_int2
...
def main():
    args = get_args(desc="Benchmark QK Int2 PV FP16 Triton")
    logger = Logger(
        name="triton_flash_attn_qk_int2_pv_fp16_" + get_save_name(args),
        log_dir="logs",
        logging_mode=True,
    )
