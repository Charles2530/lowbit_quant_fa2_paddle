...
if not compute_capabilities:
    raise RuntimeError(
        "No GPUs found. Please specify the target GPU architectures or build on a machine with GPUs."
    )
...
ext_modules = []
qattn_extension = paddle.utils.cpp_extension.CUDAExtension(
    sources=[
...
fused_extension = paddle.utils.cpp_extension.CUDAExtension(
...
setuptools.setup(
    ...
    cmdclass={"build_ext": paddle.utils.cpp_extension.BuildExtension},
)
