...
sys.path.append("/data/charles/codes/flash-attn-triton-paddle")
import math
import os
import pdb
import random
...
def _fwd_kernel(
    ...
    v_ptrs = (
        ...
        + off_b * stride_vb
        + off_h * stride_vh
        + (offs_n[:, None] * stride_vn + offs_dimv[None, :])
    )
    ...
    idx_range = tl.arange(0, BLOCK_N).expand_dims(0).broadcast_to(BLOCK_M, BLOCK_N)
    ...
    for start_n in range(0, end_n, BLOCK_N):
        ...
        start_col = tl.arange(0, BLOCK_M)
        start_row = tl.arange(0, BLOCK_N // 8) * feat_per_int
        """  
        load_scale_ptrs = TMP16 + off_hb * BLOCK_M * BLOCK_N + start_col[:, None] * KSCALE_DIM*4 + tl.arange(0, KSCALE_DIM*4)
        tl.store(load_scale_ptrs, 0.0)
        store_scale_ptrs = TMP16 + off_hb * BLOCK_M * BLOCK_N + start_col[:, None] * KSCALE_DIM*4 + tl.arange(0, KSCALE_DIM)*4
        repeat_num = (bits * group_size // 32)
        for i in range(repeat_num):
            tl.store(store_scale_ptrs+i, kscale)
        kscale = tl.load(load_scale_ptrs)
        
        load_scale_ptrs = TMP16 + off_hb * BLOCK_M * BLOCK_N + start_col[:, None] * KSCALE_DIM*4 + tl.arange(0, KSCALE_DIM*4)
        tl.store(load_scale_ptrs, 0.0)
        store_scale_ptrs = TMP16 + off_hb * BLOCK_M * BLOCK_N + start_col[:, None] * KSCALE_DIM*4 + tl.arange(0, KSCALE_DIM)*4
        for i in range(repeat_num):
            tl.store(store_scale_ptrs+i, kmn)
        kmn = tl.load(load_scale_ptrs) 
        """
        kscale = (
        ...
        load_qk_ptrs = (
            TMP32
            + off_hb * BLOCK_N * BLOCK_M
            + start_col[:, None] * BLOCK_N
            + tl.arange(0, BLOCK_N)[None, :]
        )
        tl.store(load_qk_ptrs, 0.0)
        t_qk_ptrs = (
            TMP32
            + off_hb * BLOCK_N * BLOCK_M
            + start_col[:, None] * BLOCK_N
        ...
        for idx in range(feat_per_int):
            ...
            t_qk = tl.dot(q, t_k)
            tl.store(t_qk_ptrs + idx, t_qk)
        qk = tl.load(load_qk_ptrs)
        if not EVEN_N:
        ...
        p = p.to(meta_dtype)
        start_col = tl.arange(0, BLOCK_N)
        start_row = tl.arange(0, BLOCK_HEADDIM // 8) * feat_per_int
        """ load_scale_ptrs = TMP16 + off_hb * BLOCK_M * BLOCK_N + start_col[:, None] * VSCALE_DIM*4 + tl.arange(0, VSCALE_DIM*4)
        tl.store(load_scale_ptrs, 0.0)
        store_scale_ptrs = TMP16 + off_hb * BLOCK_M * BLOCK_N+ start_col[:, None] * VSCALE_DIM*4 + tl.arange(0, VSCALE_DIM)*4
        repeat_num = (bits * group_size // 32)
        for i in range(repeat_num):
            tl.store(store_scale_ptrs+i, vscale)
        vscale = tl.load(load_scale_ptrs)
        
        load_scale_ptrs = TMP16 + off_hb * BLOCK_M * BLOCK_N + start_col[:, None] * VSCALE_DIM*4 + tl.arange(0, VSCALE_DIM*4)
        tl.store(load_scale_ptrs, 0.0)
        store_scale_ptrs = TMP16 + off_hb * BLOCK_M * BLOCK_N + start_col[:, None] * VSCALE_DIM*4 + tl.arange(0, VSCALE_DIM)*4
        for i in range(repeat_num):
            tl.store(store_scale_ptrs+i, vmn)
        vmn = tl.load(load_scale_ptrs) """
        vscale = (
        ...
        load_ao_ptrs = (
            TMP32
            + off_hb * seqlen_q_rounded * BLOCK_HEADDIM
            + start_col[:, None] * BLOCK_HEADDIM
            + tl.arange(0, BLOCK_HEADDIM)[None, :]
        )
        tl.store(load_ao_ptrs, 0.0)
        t_ao_ptrs = (
            TMP32
            + off_hb * seqlen_q_rounded * BLOCK_HEADDIM
            + start_col[:, None] * BLOCK_HEADDIM
        ...
        for idx in range(feat_per_int):
            ...
            t_ao = tl.dot(p, t_v)
            tl.store(t_ao_ptrs + idx, t_ao)
        acc_o = acc_o + tl.load(load_ao_ptrs)
        m_i = m_ij
...
def _quantized_flash_attn_forward(
    ...
    tmp32 = paddle.empty(
        (batch, nheads, BLOCK, BLOCK), device=q.place, dtype=kscale.dtype
    )
...
def _flash_attn_forward(q, k, v, bias=None, causal=False, softmax_scale=None):
    ...
    tmp = paddle.empty(
        (batch, nheads, seqlen_q_rounded, BLOCK), device=q.place, dtype=paddle.float32
    )
    o = paddle.empty_like(q)
    num_warps = 4
    grid = lambda META: (triton.cdiv(seqlen_q, META["BLOCK_M"]), batch * nheads)
...
>>>>>>        ) = flash_attn.flash_attn_interface._flash_attn_forward(
    ...
    print(
        "time for triton: %.12f, cuda: %.12f (repeat %d)"
        % ((ed_time1 - st_time1) / iter_num, (ed_time2 - st_time2) / iter_num, iter_num)
    )
...
start_values = end_values = 4, 128, 12, 128
increments = 1, 1, 1, 1
seed_all()
