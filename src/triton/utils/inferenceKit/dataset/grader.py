...
def is_polar_coordinate(expr: str) -> bool:
    """
    检查表达式是否为极坐标形式。假设极坐标通常是 (r, θ) 的形式，其中 θ 可以是 π 或角度符号。
    """
    polar_pattern = re.compile(
        "\\(\\s*\\d+(\\.\\d+)?\\s*,\\s*(pi|\\d+(\\.\\d+)?|π|tau)\\s*\\)", re.IGNORECASE
    )
...
def clean_choice_answer(prediction: str) -> str:
    """
    Clean and format a choice answer prediction.
    """
    pred = prediction.strip("\n").rstrip(".").rstrip("/").strip(" ").lstrip(":")
    choices = re.findall("\\b(A|B|C|D|E)\\b", pred.upper())
...
def parse_numeric_value(value):
    """
    Parse a string to a float, removing commas and handling percentages.
    """
...
def is_numeric(value):
    """
    Check if a string can be converted to a numeric value using parse_numeric_value.
    """
...
def latex_to_pmatrix(input_string):
    """
    Convert a LaTeX-formatted string to a pmatrix format.
    """
    input_string = input_string.strip()
    matrices = re.findall("\\{.*,.*\\}", input_string)
    pmatrix_list = [
        "\\begin{{pmatrix}}{}\\end{{pmatrix}}".format(
            matrix.strip("{}").replace(",", "\\\\")
        )
...
def simplify_fraction(expr):
    """
    Convert LaTeX fractions to a numeric division expression.
    """
    expr = re.sub("\\\\frac{(\\d+)}{(\\d+)}", "(\\1/\\2)", expr)
    expr = re.sub("\\\\dfrac{(\\d+)}{(\\d+)}", "(\\1/\\2)", expr)
    expr = re.sub("\\\\frac{(\\d+)}(\\d+)", "(\\1/\\2)", expr)
    expr = re.sub("\\\\frac(\\d+){(\\d+)}", "(\\1/\\2)", expr)
    expr = re.sub("\\\\frac(\\d)(\\d)", "(\\1/\\2)", expr)
    return expr
...
def simplify_plus_minus(expr):
    """
    Convert LaTeX \\pm expressions to their expanded form.
    """
    expr = re.sub(
        "(\\\\d+)\\\\\\\\pm(\\\\d+)\\\\\\\\sqrt{(\\\\d+)}",
...
def math_equal(
    ...
    """
    Check mathematical equality between two expressions, supporting symbolic and numeric comparisons.
    """
    ...
    try:
        if is_numeric(pred) and is_numeric(ref):
            ...
            comparisons = (
                [ref_num / 100, ref_num, ref_num * 100]
                if allow_percentage
                else [ref_num]
            )
            return any(
    ...
    ref, pred = re.sub("[{}()\\[\\]]", "", ref), re.sub("[{}()\\[\\]]", "", pred)
    ...
    if (
        ...
        and all(
            math_equal(
                part.strip(), ref_part.strip(), allow_percentage, allow_close_match
            )
        ...
        return True
    if (
        re.match("\\\\begin{(p|b)matrix}", pred)
        and re.match("\\\\begin{(p|b)matrix}", ref)
        and all(
            all(
                math_equal(
                    pred_item.strip(),
    ...
    if (
        ...
        and (
            symbolic_equal(
                f"{pred.split('=')[0].strip()} - ({pred.split('=')[1].strip()})",
        ...
        return True
    return (
        call_with_timeout(symbolic_equal_process, pred, ref)
        if check_timeout
        else symbolic_equal(pred, ref)
    )


def numeric_equal(a: float, b: float, tolerance=0.0001) -> bool:
    """
...
def symbolic_equal(a_expr, b_expr) -> bool:
    ...
    try:
        ...
        pass
    try:
        if a.shape == b.shape and a.applyfunc(lambda x: round(x, 3)).equals(
            b.applyfunc(lambda x: round(x, 3))
        ):
            return True
    except:
        pass
...
def call_with_timeout(func, *args, timeout=1, **kwargs):
    ...
    process = multiprocessing.Process(
        target=func, args=args + (output_queue,), kwargs=kwargs
    )
    process.start()
