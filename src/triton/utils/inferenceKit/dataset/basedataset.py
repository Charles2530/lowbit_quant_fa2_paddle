...
def clean_latex(s):
    s = re.sub("\\\\text\\{([^}]*)\\}", "\\1", s)
    s = re.sub("\\\\left|\\right", "", s)
    s = re.sub("\\s+", "", s)
    s = re.sub("\\\\begin\\{pmatrix\\}|\\\\end\\{pmatrix\\}", "", s)
    return s
...
class BaseDataset(paddle.io.Dataset):
    ...
    def evaluate_results(self, results, print_result=False, print_no_match=False):
        ...
        no_match_samples = 0
        print("Evaluating...\n")
        for sample in tqdm(results, desc="Submitting tasks to executor", unit="sample"):
            total_samples += 1
            response_list = str(sample.get("response", []))
            answer = str(sample.get("answer", ""))
            final_response = response_list if response_list else ""
            boxed_match = re.findall("\\\\boxed{((?:[^{}]|{[^{}]*})*)}", final_response)
    ...
    def compare(self, result_file1: str, result_file2: str):
        ...
        for item1, item2 in zip(data1, data2):
            ...
            a_response = re.search(
                "\\\\boxed{((?:(?!text{)(?:[^{}]|{[^{}]*}))*?)}", a_response
            )
            a_final_response = clean_latex(a_final_response)
            b_response = re.search(
                "\\\\boxed{((?:(?!text{)(?:[^{}]|{[^{}]*}))*?)}", b_response
            )
