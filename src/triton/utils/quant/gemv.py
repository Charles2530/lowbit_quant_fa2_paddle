...
def gemv_kernel_g64(
    ...
    for group_idx in range(0, num_groups):
        ...
        cur_qw_ptr = (
            qw_ptr
            + oc_idx[:, None] * weight_w
            + group_idx * (64 // PACK_FACTOR)
            + tl.arange(0, 64 // PACK_FACTOR)[None, :]
        )
        ...
        for i in range(PACK_FACTOR):
            ...
            cur_inp_ptr = (
                ...
                + group_idx * 64
                + i
                + tl.arange(0, 64 // PACK_FACTOR)[None, :] * PACK_FACTOR
            )
...
def gemv_fwd(bit, group_size, inp, qweight, mn, scale):
    ...
    output = paddle.empty((B, OC), device=inp.place, dtype=paddle.float16)
    grid = lambda META: (triton.cdiv(OC, META["OC_PER_PH"]), B)
    gemv_kernel_g64[grid](
        inp, qweight, mn, scale, output, IC, OC, bit, OC_PER_PH, PACK_FACTOR, BLOCK_SIZE
    )
    return output
...
def test_bgemv_outer_correct_mha():
    flatten_B = B * nh
    inp = paddle.randn((flatten_B, 1, IC), device="cuda", dtype=paddle.float16)
    ori_weight = paddle.randn((flatten_B, IC, OC), device="cuda", dtype=paddle.float16)
    GS = 32
    for BIT in [2, 4]:
        weight = ori_weight
        PACK_FACTOR = 32 // BIT
        ...
        mx = paddle.compat.max(weight, dim=-1, keepdim=False)[0]
        mn = paddle.compat.min(weight, dim=-1, keepdim=False)[0]
        maxq = 2**BIT - 1
        scale = (mx - mn) / maxq
        weight = weight - mn.unsqueeze(-1)
        weight.div_(scale.unsqueeze(-1))
        weight = weight.clip_(0, maxq).round_().to(paddle.int32)
        ...
        output = kivi_gemv.gemv_forward_cuda_outer_dim(
            inp, qweight, scale, mn, BIT, GS, nh, False
        )
        deq_w = dequant_weight_outer(
            weight.transpose(1, 2), scale.transpose(1, 2), mn.transpose(1, 2), GS
        )
        output_ref = inp @ deq_w
        error = output_ref - output
        rel_out_error = paddle.abs(
            x=error.float() / (paddle.abs(x=output_ref).float() + 1e-05)
        ).mean()
...
def test_bgemv_outer_correct_mqa():
    flatten_B = B * nh
    inp = paddle.randn((flatten_B, 1, IC), device="cuda", dtype=paddle.float16)
    ori_weight = paddle.randn((B, IC, OC), device="cuda", dtype=paddle.float16)
    GS = 32
    for BIT in [2, 4]:
        weight = ori_weight
        PACK_FACTOR = 32 // BIT
        ...
        mn = mn.transpose(1, 2).contiguous()
        output = kivi_gemv.gemv_forward_cuda_outer_dim(
            inp, qweight, scale, mn, BIT, GS, nh, True
        )
        deq_w = dequant_weight_outer(
            weight.transpose(1, 2), scale.transpose(1, 2), mn.transpose(1, 2), GS
        )
...
def test_gemv_correct():
    inp = paddle.randn((B, IC), device="cuda", dtype=paddle.float16)
    ori_weight = paddle.randn((OC, IC), device="cuda", dtype=paddle.float16)
    GS = 64
    for BIT in [4]:
        weight = ori_weight
        PACK_FACTOR = 32 // BIT
        ...
        mx = paddle.compat.max(weight, dim=2, keepdim=False)[0]
        mn = paddle.compat.min(weight, dim=2, keepdim=False)[0]
        maxq = 2**BIT - 1
        scale = (mx - mn) / maxq
        weight = weight - mn.unsqueeze(-1)
        weight.div_(scale.unsqueeze(-1))
        ...
        rel_error = paddle.abs(
            x=(deq_w - ori_weight).float() / (ori_weight + 1e-05).float()
        ).mean()
        output_ref = inp @ deq_w.T
        error = output_ref - output
        rel_out_error = paddle.abs(
            x=error.float() / (output_ref + 1e-05).float()
        ).mean()
...
def test_bgemv_outer_speed():
    inp = paddle.randn((B, 1, IC), device="cuda", dtype=paddle.float16)
    ori_weight = paddle.randn((B, IC, OC), device="cuda", dtype=paddle.float16)
    GS = 64
    for BIT in [2]:
        weight = ori_weight
        ...
        print(
            f"our fused BS {B} IC {IC} OC {OC} {BIT}-bit outer-dim batched gemv: {t_our * 1000} ms"
        )
