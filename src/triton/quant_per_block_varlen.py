...
def per_block_int8(
    ...
    cu_seqlens_q_scale = paddle.compat.pad(
        paddle.cumsum(q_scale_len, dim=0), (1, 0), value=0
    )
    cu_seqlens_k_scale = paddle.compat.pad(
        paddle.cumsum(k_scale_len, dim=0), (1, 0), value=0
    )
    q_scale = paddle.empty(
        (cu_seqlens_q_scale[-1], h_qo), device=q.place, dtype=paddle.float32
    )
    k_scale = paddle.empty(
        (cu_seqlens_k_scale[-1], h_kv), device=k.place, dtype=paddle.float32
    )
    if sm_scale is None:
