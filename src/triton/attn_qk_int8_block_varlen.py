...
def _attn_fwd_inner(
    ...
    """
    _attn_fwd_inner: compute the attention scores and update the accumulator
    triton kernel for the forward pass of the attention mechanism
    Args:
        acc: accumulator tensor
        l_i: tensor for the log sum of probabilities
        m_i: tensor for the maximum log sum of probabilities
        q: query tensor
        q_scale: query scale
        kv_len: length of the key-value pairs
        K_ptrs: pointers to the key tensors
        K_scale_ptr: pointer to the key scale tensor
        V_ptrs: pointers to the value tensors
        stride_kn: stride of the key tensor
        stride_vn: stride of the value tensor
        start_m: starting index for the m dimension
        H: number of heads
