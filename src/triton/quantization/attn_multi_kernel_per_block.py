...
def _attn_fwd_inner(
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        k_bitmap = tl.load(K_bitmap_ptr)
        k_scale = tl.load(K_scale_ptr)
        qk_out = tl.dot(q, k).to(tl.int8)
        mask = k_bitmap.to(tl.int1)
        qk_where = qk_out & ~mask | qk_out & 15 & mask
        qk = qk_where.to(tl.float32) * q_scale * k_scale
        ...
        K_bitmap_ptr += BLOCK_N * stride_bitmap
    return acc, l_i, m_i
...
def _attn_fwd(
    ...
    K_bitmap_ptr = (
        K_bitmap + (off_z * stride_kz + off_h // num_kv_groups * stride_kh) + offs_n
    )
    O_block_ptr = (
        Out
...
def forward(
    ...
    if tensor_layout == "HND":
        ...
        stride_bitmap = k_bitmap.stride(1)
        ...
        stride_bitmap = k_bitmap.stride(2)
    else:
