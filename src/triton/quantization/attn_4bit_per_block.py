...
from src.triton.quantization.helper import *
...
def _fwd_kernel(
    ...
    tl.static_print("stride_vb", stride_vb)
    tl.static_print("stride_vh", stride_vh)
    tl.static_print("stride_vn", stride_vn)
    ...
    idx_range_1 = (
        tl.arange(0, seqlen_kv).expand_dims(0).broadcast_to(BLOCK_M, seqlen_kv)
    )
    idx_range_2 = (
        tl.arange(0, BLOCK_HEADDIM).expand_dims(0).broadcast_to(BLOCK_M, BLOCK_HEADDIM)
    )
    ...
    for start_n in range(0, end_n, BLOCK_N):
        ...
        qk = tl.zeros([BLOCK_M, seqlen_kv], dtype=tl.float32)
        tl.static_print(kscale)
        tl.static_print(KSCALE_DIM)
        kscale = (
        ...
        for idx in range(feat_per_int):
            ...
            t_qk = tl.dot(q, t_k)
            tl.static_print("BLOCK_N // feat_per_int", BLOCK_N // feat_per_int)
            tl.static_print("idx_range", idx_range_1)
            t_qk = (
                t_qk.expand_dims(2)
                .broadcast_to(BLOCK_M, seqlen_kv // feat_per_int, feat_per_int)
                .reshape(BLOCK_M, seqlen_kv)
            )
            tl.static_print("qk", qk)
            tl.static_print("t_qk", t_qk)
            qk = tl.where(idx_range_1 % feat_per_int == idx, t_qk, qk)
        if not EVEN_N:
        ...
        p = p.to(meta_dtype)
        tl.static_print("vscale", vscale)
        vscale = (
            vscale.expand_dims(2)
            .broadcast_to(seqlen_kv, VSCALE_DIM, 4)
            .reshape(seqlen_kv, VSCALE_DIM * 4)
        )
        vmn = (
            vmn.expand_dims(2)
            .broadcast_to(seqlen_kv, VSCALE_DIM, 4)
        ...
        for idx in range(feat_per_int):
            ...
            tl.static_print("p", p)
            tl.static_print("t_v", t_v)
            tl.static_print("vscale", vscale)
            tl.static_print("vmn", vmn)
            t_v = tl.fma(t_v.to(meta_dtype), vscale, vmn)
            t_ao = tl.dot(p, t_v)
            tl.static_print("t_ao", t_ao)
            tl.static_print("acc_o", acc_o)
            t_ao = (
                t_ao.expand_dims(2)
                .broadcast_to(BLOCK_M, BLOCK_HEADDIM // feat_per_int, feat_per_int)
                .reshape(BLOCK_M, BLOCK_HEADDIM)
            )
            tl.static_print("acc_o", acc_o)
            tl.static_print("t_ao", t_ao)
            tl.static_print("idx_range", idx_range_2)
            acc_o = tl.where(idx_range_2 % feat_per_int == idx, t_ao, acc_o)
        m_i = m_ij
...
def main(tree_width, seq_len, head_num, feat_dim):
    ...
    from src.triton.utils.quant.new_pack import (
        triton_quantize_and_pack_along_last_dim, unpack_and_dequant_kcache,
