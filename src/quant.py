...
def per_warp_int8(
    ...
    _tensor_layout = 0 if tensor_layout == "NHD" else 1
    q_scale = paddle.empty(
        (b, h_qo, (qo_len + 127) // 128 * (128 // 32)),
        device=q.place,
        dtype=paddle.float32,
    )
    k_scale = paddle.empty(
        (b, h_kv, (kv_len + 63) // 64), device=q.place, dtype=paddle.float32
    )
    _fused.quant_per_warp_int8_cuda(q, q_int8, q_scale, _tensor_layout)
    if km is not None:
        km = km.squeeze(1) if _tensor_layout == 0 else km.squeeze(2)
        _fused.quant_per_block_int8_fuse_sub_mean_cuda(
            k, km, k_int8, k_scale, 64, _tensor_layout
        )
    else:
...
def sub_mean(v: paddle.Tensor, tensor_layout: str = "HND"):
    ...
    v_smoothed = paddle.empty(v.shape, dtype=paddle.float16, device=v.place)
...
def per_channel_fp8(
    ...
    if tensor_layout == "HND":
        ...
        v_transposed_permutted = paddle.empty(
            (b, h_kv, head_dim, padded_len), dtype=v.dtype, device=v.place
        )
    elif tensor_layout == "NHD":
        b, kv_len, h_kv, head_dim = v.shape
        padded_len = (kv_len + 63) // 64 * 64
        v_transposed_permutted = paddle.empty(
            (b, head_dim, h_kv, padded_len), dtype=v.dtype, device=v.place
        )
    _fused.transpose_pad_permute_cuda(v, v_transposed_permutted, _tensor_layout)
    v_fp8 = paddle.empty(
# >>>>>>        v_transposed_permutted.shape, dtype=torch.float8_e4m3fn, device=v.place
    ...
    if smooth_v:
        _fused.mean_scale_fuse_quant_cuda(
