...
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim, DataType DTypeQK, QuantGranularity Q_GRAN, QuantGranularity K_GRAN,
         typename DTypeOut = half, ComputeUnit DenominatorAccumUnit, MaskMode mask_mode = MaskMode::kNone, uint32_t Buffer_Iter = 16, bool return_lse = false>
__global__ void kv_guided_qk_int8_sv_f16_accum_f16_attn_buf(int8_t *__restrict__ Q, int8_t *__restrict__ K, half *__restrict__ V, DTypeOut *__restrict__ O, float *__restrict__ Lse,
...
{
  // compile time check
  // Q address = Q=0x7fff2ec00000, K 0x7fff2b901000, V 0x7fff2b880000
  ...
  // const uint32_t batch_id = blockIdx.z;
  // block: (2, 32, 1)
  const uint32_t bx = blockIdx.x; // Key 的 block 索引, klen/CTA_K
  const uint32_t num_kv_heads = gridDim.y;  // 32
  const uint32_t head_id = blockIdx.y; //  4

  // transfer to base 2 instead of base e with better numerical efficiency
  sm_scale *= math::log2e;

  // RS holds the fragment of S
  int32_t RS[batch_size * num_tiles_q][num_tiles_k][8];
  half RO[batch_size * num_tiles_q][num_tiles_v][8];
  float m[batch_size * num_tiles_q][2]; // max
  float d[batch_size * num_tiles_q][2]; // denominator

  float m_buf[batch_size * num_tiles_q][2]; // buffer for m
  float RO_buf[batch_size * num_tiles_q][num_tiles_v][8]; // buffer for RO
  ...
  if constexpr (Q_GRAN == QuantGranularity::kPerBlock)
  {
      ...
      q_scale_idx = head_id * num_block_q;
  }
  else if constexpr (Q_GRAN == QuantGranularity::kPerWarp)
  {
      const uint32_t num_warp_block_q = div_ceil(qo_len, CTA_Q) * (CTA_Q / WARP_Q);
      // q_scale_idx = batch_size * num_qo_heads * num_warp_block_q + head_id * num_warp_block_q + get_warp_idx_q<num_warps_q, num_warps_k>();
      q_scale_idx = head_id * num_warp_block_q + get_warp_idx_q<num_warps_q, num_warps_k>();
  } 
  else if constexpr (Q_GRAN == QuantGranularity::kPerThread)
  {
      const uint32_t num_warp_block_q = div_ceil(qo_len, CTA_Q) * (CTA_Q / WARP_Q);
      q_scale_idx = batch_size * num_qo_heads * (num_warp_block_q * 8) + head_id * (num_warp_block_q * 8) +
  ...
  {
      // 每个块处理一部分 Key
      const uint32_t num_block_k = gridDim.x; // 2 
      k_scale_idx = head_id * num_block_k + bx;
  }
  else if constexpr (K_GRAN == QuantGranularity::kPerWarp)
  {
      // 每个 Warp 处理一部分 Key
      const uint32_t num_warp_block_k = gridDim.x * num_warps_k; // 每个 Key 块再细分为 Warp 级别
      k_scale_idx = head_id * num_warp_block_k + bx * num_warps_k + get_warp_idx_k<num_warps_q, num_warps_k>();
  }
  else if constexpr (K_GRAN == QuantGranularity::kPerThread)
  {
      // 每个线程处理一个 Key
      const uint32_t num_thread_block_k = gridDim.x * num_warps_k; // 每个 Key 块再细分为线程级别
      k_scale_idx = head_id * num_thread_block_k * 4 + bx * num_warps_k * 4 + get_warp_idx_k<num_warps_q, num_warps_k>() * 4 + lane_id % 4; 
  }

  constexpr uint32_t q_scale_advance_offset = (Q_GRAN == QuantGranularity::kPerBlock) ? 1 
        : (Q_GRAN == QuantGranularity::kPerWarp) ? (CTA_Q / WARP_Q) : (CTA_Q / WARP_Q) * 8;

    // initialize o, m, d
  #pragma unroll
    for (uint32_t fq = 0; fq < num_tiles_q * batch_size; fq++)
    {
  #pragma unroll
  ...
  constexpr uint32_t global_to_shared_line_lanes_QK = (QK_SMEM_STRIDE == 32) ? 2 : (QK_SMEM_STRIDE == 64) ? 4 : 8; // 8
  constexpr uint32_t global_to_shared_copy_lines_per_warp_QK = (QK_SMEM_STRIDE == 32) ? 16 : (QK_SMEM_STRIDE == 64) ? 8 : 4; // 4
  ...
  constexpr uint32_t QK_smem_iters_row = QK_SMEM_STRIDE / (global_to_shared_line_lanes_QK * PACK_SIZE_QK); // head_dim / 8 = 128/8=16
  constexpr uint32_t Q_smem_iters_col = CTA_Q / (num_warps * global_to_shared_copy_lines_per_warp_QK); // 128/(4*1*4)=8
  constexpr uint32_t K_smem_iters_col = CTA_K / (num_warps * global_to_shared_copy_lines_per_warp_QK); // 64/(4*1*4)=4
  ...
  int8_t *Q_lane_base_ptr = Q + head_id * stride_h_q + (CTA_Q / num_warps * warp_id + lane_id / global_to_shared_line_lanes_QK) * stride_seq_q + (lane_id % global_to_shared_line_lanes_QK) * PACK_SIZE_QK;
  int8_t *K_lane_base_ptr = K + (head_id) * stride_h_k + (bx * CTA_K + CTA_K / num_warps * warp_id + lane_id / global_to_shared_line_lanes_QK) * stride_seq_k + (lane_id % global_to_shared_line_lanes_QK) * PACK_SIZE_QK;
  half *V_lane_base_ptr = V + (head_id) * stride_h_v + (bx * CTA_K + CTA_K / num_warps * warp_id + lane_id / global_to_shared_line_lanes_V) * stride_seq_v + (lane_id % global_to_shared_line_lanes_V) * PACK_SIZE_V;
  ...
  // for causal masking
  uint32_t Q_idx_lane_base = get_warp_idx_q<num_warps_q, num_warps_k>() * WARP_Q + lane_id / 4;
  uint32_t K_idx_lane_base = bx * CTA_K + get_warp_idx_k<num_warps_q, num_warps_k>() * WARP_K + 2 * (lane_id % 4);

  // for loading
  uint32_t Q_load_idx_lane_base = CTA_Q / num_warps * warp_id + lane_id / global_to_shared_line_lanes_QK;
  uint32_t K_load_idx_lane_base = bx * CTA_K + CTA_K / num_warps * warp_id + lane_id / global_to_shared_line_lanes_QK;
  uint32_t V_load_idx_lane_base = bx * CTA_K + CTA_K / num_warps * warp_id + lane_id / global_to_shared_line_lanes_V;
  ...
  if (CTA_Q >= head_dim) {
    Q_load_idx_lane_base += (num_qo_heads * qo_len * head_dim); // advance to next block, CTA_Q = 128
  } else {
    Q_load_idx_lane_base += CTA_Q; // advance to next block, CTA_Q = 128
  }
  ...
  {
    std::cout << "num_tiles_qk_inner == 1" << std::endl;
#pragma unroll
  ...
  float k_scale = K_scale[k_scale_idx];

  float original_sm_scale = sm_scale;
  float dequant_scale = Q_scale[q_scale_idx + 0 * q_scale_advance_offset * num_qo_heads] * k_scale;
  ...
  K_load_idx_lane_base += CTA_K; // CTA_K = 64
...
#pragma unroll
    for (; iter <= (flush_time + 1) * Buffer_Iter; iter++)
    {
      ...
      // 对 query 的 batch_size 的循环
      // for (uint32_t cnt_b = 0; cnt_b < batch_size; cnt_b++)
      uint32_t cnt_b = iter % batch_size;
      // ensure K is ready
      cp_async::wait_group<1>();
      __syncthreads();

      // compute QK^T, 算的是 all Qs 和当前 K 的 qk 结果
      if constexpr (num_tiles_qk_inner == 1)
      {
  ...
  #pragma unroll
      ...
      if (cnt_b == 7)
      {
        K_idx_lane_base += CTA_K; // CTA_K = 64
        ...
        dequant_scale = Q_scale[q_scale_idx + cnt_b * q_scale_advance_offset * num_qo_heads] * k_scale;
        sm_scale = original_sm_scale * dequant_scale;
...
#pragma unroll
  ...
  #pragma unroll
        for (uint32_t k = 0; k < 2; k++)
        {
          float o_scale = math::ptx_exp2(m_buf[bs*fq][k] - m[bs*fq][k]);
  #pragma unroll
          for (uint32_t fv = 0; fv < num_tiles_v; fv++)
          {
            // update buffer
            RO_buf[bs*fq][fv][k * 2 + 0] = RO_buf[bs*fq][fv][k * 2 + 0] * o_scale + __half2float(RO[bs*fq][fv][k * 2 + 0]);
            RO_buf[bs*fq][fv][k * 2 + 1] = RO_buf[bs*fq][fv][k * 2 + 1] * o_scale + __half2float(RO[bs*fq][fv][k * 2 + 1]);
            RO_buf[bs*fq][fv][k * 2 + 4] = RO_buf[bs*fq][fv][k * 2 + 4] * o_scale + __half2float(RO[bs*fq][fv][k * 2 + 4]);
            RO_buf[bs*fq][fv][k * 2 + 5] = RO_buf[bs*fq][fv][k * 2 + 5] * o_scale + __half2float(RO[bs*fq][fv][k * 2 + 5]);

            // update m_buf
            m_buf[bs*fq][k] = m[bs*fq][k];

            // clear RO
            *((int32_t*)&RO[bs*fq][fv][k * 2 + 0]) = 0;
            *((int32_t*)&RO[bs*fq][fv][k * 2 + 4]) = 0;
          }
        }
      }
    }
...
#pragma unroll
    ...
    dequant_scale = Q_scale[q_scale_idx + iter * q_scale_advance_offset] * k_scale;
    sm_scale = original_sm_scale * dequant_scale;
...
#pragma unroll
  ...
  normalize_d<num_tiles_q, num_tiles_v, DenominatorAccumUnit>(RO_buf, m, d);
...
#pragma unroll
  ...
  DTypeOut *O_lane_ptr = O + head_id * stride_h_o + (bx * CTA_Q + WARP_Q * get_warp_idx_q<num_warps_q, num_warps_k>() + lane_id / global_to_shared_line_lanes_O) * stride_seq_o + lane_id % global_to_shared_line_lanes_O * PACK_SIZE_O;
...
#pragma unroll
  ...
  { 
    ...
    float *lse_lane_ptr = Lse + head_id * qo_len + lse_idx;
...
torch::Tensor kv_guided_qk_int8_sv_f16_accum_f16_attn_buf(torch::Tensor query,
                    torch::Tensor key,
                    torch::Tensor value,
                    torch::Tensor output,
                    ...
                    int return_lse)
{
  ...
  if (tensor_layout == 0)
  {
    ...
    CHECK_SHAPE(key, 1, kv_len, num_kv_heads, head_dim);
    CHECK_SHAPE(value, 1, kv_len, num_kv_heads, head_dim);
  ...
  else if (tensor_layout == 1)
  {
    ...
    CHECK_SHAPE(key, 1, num_kv_heads, kv_len, head_dim);
    CHECK_SHAPE(value, 1, num_kv_heads, kv_len, head_dim);
        ...
        DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, {
          DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(output_dtype, DTypeOut, {
            ...
            if constexpr (QK_QUANT_GRAN == static_cast<int>(QuantGranularity::kPerWarp))
            {
              ...
              CHECK_SHAPE(key_scale, 1, num_kv_heads, static_cast<long>(div_ceil(kv_len, CTA_K) * (CTA_K / WARP_K)));
            }
            else if constexpr (QK_QUANT_GRAN == static_cast<int>(QuantGranularity::kPerThread))
            {
              ...
              CHECK_SHAPE(key_scale, 1, num_kv_heads, static_cast<long>(div_ceil(kv_len, CTA_K) * (CTA_K / WARP_K) * 4));
            }
            else
            {
            ...
            auto kernel_func = kv_guided_qk_int8_sv_f16_accum_f16_attn_buf<CTA_Q, CTA_K, WARP_Q, WARP_K, HEAD_DIM, DataType::kInt8, 
                    static_cast<QuantGranularity>(QK_QUANT_GRAN), static_cast<QuantGranularity>(QK_QUANT_GRAN), DTypeOut, ComputeUnit::kTensorCore, 
                    mask_mode, 32, RETURN_LSE>; 
            ...
            dim3 grid(div_ceil(kv_len, CTA_K), num_kv_heads, 1);  // cuda block (2, 32, 1)
            dim3 block(32, (CTA_Q / WARP_Q) * (CTA_K / WARP_K));  // cuda thread (32, 4, 1)
